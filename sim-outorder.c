...
// ------------------------------------------------------------------------------------------------------------------------------
//                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5: MEMORY DEPENDENCE ALGORITHM SELECTION 
// Description: The selectable memory dependence algorithms are presented below with the corresponding numeric specifier.
//               For example, "./sim-outorder -ALGORITHM_TYPE 3 ./tests/bin.little/test-llong" runs sim-outorder with the store sets algorithm.
// ------------------------------------------------------------------------------------------------------------------------------
//  ALGORITHM_TYPE
//  1 = NO SPECULATION                for calling lsq_refresh_NoSpeculation()
//  2 = NAIVE SPECULATION             for calling lsq_refresh_NaiveSpeculation()
//  3 = STORE SETS                    for calling lsq_refresh_InfStoreSets()
//  4 = PREDICTION DEPENDENCE COUNTER for calling lsq_refresh_CountingDependencePredictor()
static unsigned int ALGORITHM_TYPE = 0;
// ------------------------------------------------------------------------------------------------------------------------------
//                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5: GLOBAL PERFORMANCE METRICS
// Description: The following metrics are used to compare the performane of various memory dependence algorithms against eachother. 
// ------------------------------------------------------------------------------------------------------------------------------
static int numNotPredicted       = 0;
static int numCorrectlyPredicted = 0;
static int numFalseDependencies  = 0;
static int numMemViolations      = 0; 
// ------------------------------------------------------------------------------------------------------------------------------
//                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5: INFINITE STORE SETS INITIALIZATIONS
// Description: Store Set Parameter Initialization
// ------------------------------------------------------------------------------------------------------------------------------
#define MAX_STORESET_SIZE       1000 // Effectively Infinite
#define MAX_NUM_STORESETINDICES 1000 // Effectively Infinite
static int numStoreSetIndices = 0;
/* Store-Set Index for each Load */
struct StoreSetIndex
{
  md_addr_t loadPC;                       // PC for this Load
  md_addr_t storeSet[MAX_STORESET_SIZE];  // Contains the PCs of Stores this Load will wait for.
  int       numStores;                    // Number of Stores in the store set.
};
static struct StoreSetIndex StoreSetIndices[MAX_NUM_STORESETINDICES];
// ------------------------------------------------------------------------------------------------------------------------------
//                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5: CDP INITIALIZATIONS
// Description: Counting Dependence Predictor Parameter Initialization
// ------------------------------------------------------------------------------------------------------------------------------
#define MAX_NUM_CDPINDIES 10000   // Effectively Infinite
static int STORES_TO_WAIT_ON = 1; // Default 1 is set according to CDP paper
static int numCDPIndices     = 0;
enum CDP_STATES_t
{ //states for the CDP 
	AGGRESSIVE,  // State 00 GRRRRRRR!!!!!!1
	N_STORE,     // STATE 01
	N_STORE2,    // STATE 10 // there are two One stores as outlined in the paper
	CONSERVATIVE // STATE 11 
}; 
struct CountingDependencePredictorIndex
{
	md_addr_t loadPC;            // PC for this Load
	enum CDP_STATES_t CDP_STATE; // State added for the counting dependence predictor -- init to AGGRESSIVE!!! AS PER PAPER!!!
	int match;                   // Number of stores that may match this load's address
	int counted;                 // Should be false until the summing function
};
static struct CountingDependencePredictorIndex CountingDependencePredictorIndices[MAX_NUM_CDPINDIES];
...
/* Load/store spec exec -- number of memory violations in store set -JWG*/
// static counter_t numMemViolations; 
...
// ------------------------------------------------------------------------------------------------------------------------------
//                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5: REPORT SELECTED ALGORITHM TYPE
// Description: Print to the screen which memory prediction algorithm was used.
// ------------------------------------------------------------------------------------------------------------------------------
	//  ALGORITHM_TYPE
	//  1 = NO SPECULATION
	//  2 = NAIVE SPECULATION
	//  3 = STORE SETS
	//  4 = PREDICTION DEPENDENCE COUNTER
  // ALGORITHM_TYPE // 
  opt_reg_uint(odb, "-ALGORITHM_TYPE", "ALGORITHM Type related to Memory Dependence Prediction to execute", &ALGORITHM_TYPE, /* default */ 0,/* print */TRUE, /* format */NULL);
...
// ------------------------------------------------------------------------------------------------------------------------------
//                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5: REPORTING PERFORMANCE METRICS
// Description: Print to the screen the total number of occurances of memory violations and false dependencies during program execution.
//              Memory Violation: A ready load queues before a previous store to the same (conflicting) address finishes executing.
//              False Dependency: A ready load does not queue (i.e. stalled) in order to wait for a non-ready store(s) which does not actually conflict. 
// ------------------------------------------------------------------------------------------------------------------------------
  stat_reg_counter(sdb, "numMemViolations", "Number of memory-order violations",
                   &numMemViolations, /* initial value */0, /* format */NULL);
  stat_reg_counter(sdb, "numFalseDependencies", "Number of false dependencies in store set",
                   &numFalseDependencies, /* initial value */0, /* format */NULL);
...
// ------------------------------------------------------------------------------------------------------------------------------
//                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5: FUNCTION IMPLEMENTATION
// Name: countNumFalseDependencies()
// Description: Count the number of false dependencies existing in this clock cycle, and update the global counter. 
// ------------------------------------------------------------------------------------------------------------------------------
static void
countNumFalseDependencies()
{
  int i, index;
  int unresolvedStoreFound = 0;
  /* Scan the entire LSQ for non-ready stores. If any ready loads are found afterwards, increment numFalseDependencies.*/
  for (i=0, index=LSQ_head;
       ...
       i++, index=(index + 1) % LSQ_size)
    {
      ...
      {
        if (!STORE_ADDR_READY(&LSQ[index]))     // Store Address Not Ready
        {
          unresolvedStoreFound = 1;
        }
        else if (!OPERANDS_READY(&LSQ[index])) // Operands Not Ready
        {
          unresolvedStoreFound = 1;
        }
      }
      ...
      {
        if (unresolvedStoreFound)
        {
            numFalseDependencies++;
        }
      }
    }
    return;
...
// ------------------------------------------------------------------------------------------------------------------------------
//                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5: FUNCTION IMPLEMENTATION
// Name: lsq_refresh_NoSpeculation
// Description: Update the load-store-queue using no memory dependence speculation. This means it will
//              break on both Unready Addresses and Unready Operands without checking for actual address conflicts. 
// ------------------------------------------------------------------------------------------------------------------------------
static void
lsq_refresh_NoSpeculation(void)
{
  int i, j, index;

  /* scan entire queue for ready loads: scan from oldest instruction (head) until we reach the tail 
     or an unresolved store, after which no other instruction will become ready */
  for (i=0, index=LSQ_head;
       i < LSQ_num;
       i++, index=(index + 1) % LSQ_size)
    {
        ...
        (MD_OP_FLAGS(LSQ[index].op) & (F_MEM|F_STORE)) == (F_MEM|F_STORE))
        {
          if (!STORE_ADDR_READY(&LSQ[index]) || !OPERANDS_READY(&LSQ[index])) 
          {
            // sta or std unknown, blocks all later loads, stop search 
            break;
          }   
        }
...
} // lsq_refresh_NoSpeculation

// ------------------------------------------------------------------------------------------------------------------------------
//                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5 FUNCTION
// Name: lsq_refresh_NaiveSpeculation()
// Description: Update the load-store-queue with naive memory dependence speculation. This means if a load is ready to queue, naively
//              assume there are NO conflicts and rely on downstream checks to identify and recover from memory dependence violations.
// ------------------------------------------------------------------------------------------------------------------------------
static void
lsq_refresh_NaiveSpeculation(void)
{
  int i, j, index;
  
  /* scan entire queue for ready loads: scan from oldest instruction
     (head) until we reach the tail, after which no other instruction will become ready */
  for (i=0, index=LSQ_head; i < LSQ_num; i++, index=(index + 1) % LSQ_size)
  {
    if (/* load? */
    ((MD_OP_FLAGS(LSQ[index].op) & (F_MEM|F_LOAD)) == (F_MEM|F_LOAD))
    && /* queued? */!LSQ[index].queued
...
} // lsq_refresh_NaiveSpeculation


// ------------------------------------------------------------------------------------------------------------------------------
//                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5 FUNCTION
// Name: lsq_refresh_InfStoreSets()
// Description: Update the load-store-queue with speculation based on an infinite store set. This means that the store set may contain
//              an infinite number of stores, and a given store can exist in an infinite number of different store sets.
// ------------------------------------------------------------------------------------------------------------------------------
#define MAX_UNKNOWN_STORES    64
static void
lsq_refresh_InfStoreSets(void)
{
  int i, j, index, numUnreadyStores;
  // md_addr_t unreadyStoreAddr[MAX_STD_UNKNOWNS];
  md_addr_t unreadyStorePC[MAX_UNKNOWN_STORES]; // Keep track of Un-Ready stores PCs found within the LSQ during this clock cycle. 
  int iStoreSetIndex;
  ...
  /* scan entire queue for ready loads: scan from oldest instruction (head) until we reach the tail or an unresolved store, after which no
     other instruction will become ready */
  for (i=0, index=LSQ_head, numUnreadyStores=0;i < LSQ_num;i++, index=(index + 1) % LSQ_size)
    { 
      /* terminate search for ready loads after first unresolved store,
        as no later load could be resolved in its presence */
      if (/* store? */(MD_OP_FLAGS(LSQ[index].op) & (F_MEM|F_STORE)) == (F_MEM|F_STORE))
      {
        if (!STORE_ADDR_READY(&LSQ[index]) || !OPERANDS_READY(&LSQ[index]))
        {
            if (numUnreadyStores == MAX_UNKNOWN_STORES)
                fatal("Unready Store array overflow, increase MAX_UNKNOWN_STORES");
            // unreadyStoreAddr[numUnreadyStores++] = LSQ[index].addr;
            unreadyStorePC[numUnreadyStores++]  = LSQ[index].PC;
        }
        else /* STORE_ADDR_READY() && OPERANDS_READY() */
        {
          /* a later STD known hides an earlier STD unknown */
          for (j = 0; j < numUnreadyStores; j++)
          {
            if (unreadyStorePC[j] == /* STA/STD known */LSQ[index].PC)
              unreadyStorePC[j] = /* bogus PC */0;
          }
        }
      }
      if (/* load? */
        ...
        && /* regs ready? */OPERANDS_READY(&LSQ[index]))
      {
          // I. Find the StoreSetIndex for this particular Load
          int foundStoreSetIndex = FALSE;
          int foundMatchPC = FALSE;
          for ( iStoreSetIndex = 0; iStoreSetIndex < numStoreSetIndices; iStoreSetIndex++)
          {
            // Found a Store Set Index with a Matching PC
            if ( StoreSetIndices[iStoreSetIndex].loadPC == LSQ[index].PC)
            {
                // iStoreSetIndex -> is the StoreSetIndices index for this load.
                foundStoreSetIndex = TRUE;
                break;
            }
          }

          // II. If found, check for an unready store PC within the load's store set.
          if (foundStoreSetIndex) 
          { 
            int iStorePC;
            // Search for Match between unreadyStorePC and Load's Store Set PCs
            for ( iStorePC = 0; iStorePC < StoreSetIndices[iStoreSetIndex].numStores; iStorePC++)
            {
              // Get PC of the store instruction in the array
              md_addr_t storePC = StoreSetIndices[iStoreSetIndex].storeSet[iStorePC];
              int iUnreadyStorePC;
              for ( iUnreadyStorePC = 0; iUnreadyStorePC < numUnreadyStores; iUnreadyStorePC++)
              {
                // Check if an unready store PC matches one of the store set PCs
                if (unreadyStorePC[iUnreadyStorePC] ==  storePC)
                {
                    foundMatchPC = TRUE;
          ...
          // III. No Predicted Conflicts (or a store set does not yet exist for this load), put load on ready queue
          if (!foundMatchPC)
          {
              readyq_enqueue(&LSQ[index]);    
          } // if match not found
...
} // lsq_refresh_InfStoreSets


// ------------------------------------------------------------------------------------------------------------------------------
//                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5 FUNCTION
// Name: lsq_refresh_CountingDependencePredictor()
// Description: Update the load-store-queue with speculation based on a counting dependence predictor. The CDP only cares about the number of stores and not a perticular store. 
//              This is due to lack of information available to each node in a distributed computing network. The store PC may not be availible to each node states for the CDP
//             AGGRESSIVE,  // State 00 // Just go ahead and execute (identical to naive speculation).
//             ONE-STORE,   // STATE 01 // N matching stores arriving before or after load
//             ONE-STORE2,  // STATE 10 // N matching stores arriving before or after load
//             CONSERVATIVE // STATE 11 // loads must wait until all previous stores (in program order) have completed.
// ------------------------------------------------------------------------------------------------------------------------------
static void
lsq_refresh_CountingDependencePredictor(void)
{ 
  // Flush:  A dynamic store is a another "instance" of the orginal store. Should have same PC
  //         A load violation occurs when a load executes before an older store to the same address -- same as numMemoryViolations -- causes a flush
  int i, j, index;
  // count the number of stores ahead of each load.
  int NumPrecedingStoresPerLoad = 0; // init to zero everytime this function is called
  /* scan entire queue for ready loads: scan from oldest instruction
     (head) until we reach the tail or an unresolved store, after which no
     other instruction will become ready */
  for (i=0, index=LSQ_head; i < LSQ_num; i++, index=(index + 1) % LSQ_size)
  { 
      ...
      if (/* store? */(MD_OP_FLAGS(LSQ[index].op) & (F_MEM|F_STORE)) == (F_MEM|F_STORE))
      {
      	// if we have a store instuction count it.
      	// this counter will be reset by the load that follows.
      	NumPrecedingStoresPerLoad = NumPrecedingStoresPerLoad +1; 
        if (!STORE_ADDR_READY(&LSQ[index]) || !OPERANDS_READY(&LSQ[index]))
        {
     ...
     //////////////////////////////LOAD /////////////////////////////////////////////////////////////////////////
      if (/* load? */((MD_OP_FLAGS(LSQ[index].op) & (F_MEM|F_LOAD)) == (F_MEM|F_LOAD))
        && /* queued? */!LSQ[index].queued
        && /* waiting? */!LSQ[index].issued
        && /* completed? */!LSQ[index].completed
        && /* regs ready? */OPERANDS_READY(&LSQ[index]))
      { // this is a load in the LSQ queue 

	      	int LoadINDEX = -1; // negitive 1 is the fail value
	      	int iLoadPC =0;
	      	int OK_TO_QUEUE = 0;

	      	for ( iLoadPC = 0; iLoadPC < numCDPIndices; iLoadPC++)
	      	{ // scan the CDP list for this load's CDP struct
	      	  // PC was set in the memory violations function
	      		// printf("About to check PC with CDP entry  %08x \n", (int)LSQ[index].PC);
	      		// printf("numCDPIndices : %i\n", numCDPIndices);
	      		if(numCDPIndices == MAX_NUM_CDPINDIES)
		        {
		          	printf("numCDPIndices : %i\n", numCDPIndices);
		          	fatal("Increase the MAX_NUM_CDPINDIES");
		          	//numCDPIndices
		        }           
	      		if(CountingDependencePredictorIndices[iLoadPC].loadPC == LSQ[index].PC)
	      		{ // if the LOAD has an entry in the CDP find the index and store it.
	      			LoadINDEX = iLoadPC;
	      			break;
	      		}	      		
	      	} // we leave iLoadPC.. iLoadPC contains the index of this load
	      	 

	      	if(LoadINDEX != -1) // if we have a valid entry
	      	{ 	//STORES_TO_WAIT_ON

	      		 // loads may be enqueued if the following 
		          // ----- LOADS are in AGGRESSIVE MODE
		          // ----- LOADS ARE in CONSERVATIVE MODE and all previous stores have finished
		          // ----- LOADS in N store mode must wait for N previous stores to execute

	      		if( /*Queue loads*/CountingDependencePredictorIndices[LoadINDEX].CDP_STATE == AGGRESSIVE)
	      		{ // in aggressive mode just ship the loads out. we do not care about how many stores are ahead.
	      			OK_TO_QUEUE =1;
	      			//printf("readyq_enqueue AGGRESSIVE %i\n", (int)LSQ[index].PC);
	      		}
	      		else if ((CountingDependencePredictorIndices[LoadINDEX].CDP_STATE == N_STORE) || (CountingDependencePredictorIndices[LoadINDEX].CDP_STATE == N_STORE2))
	      		{
	      			if(NumPrecedingStoresPerLoad < STORES_TO_WAIT_ON) 
	 				  { // we have waited on all possible dependent stores
	 				  	// ie if STORES_TO_WAIT_ON is set to 3 and 2 stores remain then we may issue the load
	 				  	//readyq_enqueue(&LSQ[index]);  
	 				  	OK_TO_QUEUE = 1;
	 				  	//printf("readyq_enqueue N_STORE %i\n", (int)LSQ[index].PC);
	 				  }
	 			}
	 			// Conservative mode is just in program order issuing. so we just need to check for zero preceeding stores
	 			else if (CountingDependencePredictorIndices[LoadINDEX].CDP_STATE == CONSERVATIVE)
	 			//{
	 				if(NumPrecedingStoresPerLoad <= 0) // all previous stores have issued
	 				{
	 					//printf("readyq_enqueue CONSERVATIVE %i\n", (int)LSQ[index].PC);
			    		//readyq_enqueue(&LSQ[index]);
			    		OK_TO_QUEUE =1;
			    	}    
			    //} 
		 	...
		 	else if(LoadINDEX == -1) // LoadINDEX == -1 // something went wrong!!!
		 	{
		 	// Initialize a CDP Index for this load
		 	  numCDPIndices++;
	          CountingDependencePredictorIndices[numCDPIndices].loadPC =  LSQ[index].PC; // init an entry
	          CountingDependencePredictorIndices[numCDPIndices].match = 0; //init to zero since we don't yet have an entry
	          CountingDependencePredictorSTATEUPDATE(&CountingDependencePredictorIndices[LoadINDEX],0); // update the state machine
	          
		 	}

		 	if(OK_TO_QUEUE)
		 	{
		 		printf("readyq_enqueue %08x STATE: %i\n", (int)LSQ[index].PC , (int)CountingDependencePredictorIndices[LoadINDEX].CDP_STATE);
		 		readyq_enqueue(&LSQ[index]);
		 	}
		 		OK_TO_QUEUE =0; 

		 		//reset the NumPrecedingStoresPerLoad -- we only want to count the stores in front of each load.
		 		//LD SD SD SD LD SD SD LD-- the last load (l to r ) should get a NumPrecedingStores of 2 not 5
		 		 NumPrecedingStoresPerLoad = 0;
	        // if readyu

	    } 
    } // for LSQ index
} // lsq_refresh_CountingDependencePredictor

// ------------------------------------------------------------------------------------------------------------------------------
//                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5 FUNCTION
// Name: CountingDependencePredictorSTATEUPDATE()
// Description: Supporting function to lsq_refresh_CountingDependencePredictor(). Update the state of an input CDP. The state will determin prediction strategy.
// ------------------------------------------------------------------------------------------------------------------------------
void CountingDependencePredictorSTATEUPDATE(struct CountingDependencePredictorIndex * CDPLOAD, int flush)
{
	// match may need to be carried with the instruction 
	switch(CDPLOAD->CDP_STATE)
	{
		...
		case AGGRESSIVE:
		{
			if(flush)
			{
				CDPLOAD->CDP_STATE = CONSERVATIVE;
			}
			break;
		}
		case N_STORE:
		{
			...
			if(CDPLOAD->match == 1)
			{
				CDPLOAD->CDP_STATE = N_STORE2;
			}
			else if(CDPLOAD->match == 0)
			{
				CDPLOAD->CDP_STATE = AGGRESSIVE;
			}
		...
		default:
		{
			fatal("Load CDP is in unknown state");
		}
	} // end switch 
}
...
// ------------------------------------------------------------------------------------------------------------------------------
//                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5 FUNCTION
// Name: check_mem_violation(struct RUU_station * rs)
// Description: Detect the presence of a queued load the address of which conflicts with an un-executed store.
// ------------------------------------------------------------------------------------------------------------------------------
static void check_mem_violation(struct RUU_station * rs)
{
  // TODO:instead of break, create boolean to only count 
  //      violation once and loop through for more stores
  int j, jIndex;
  /* scan entire queue for ready loads: scan from oldest instruction
  (head) until we reach the tail or an unresolved store, after which no
  other instruction will become ready */
  for (j=0, jIndex=LSQ_head; j < LSQ_num; j++, jIndex=(jIndex + 1) % LSQ_size)
  {
    if (rs->PC == LSQ[jIndex].PC)
      break;
    /* terminate search for ready loads after first unresolved store,
    as no later load could be resolved in its presence */
    if (/* store? */(MD_OP_FLAGS(LSQ[jIndex].op) & (F_MEM|F_STORE)) == (F_MEM|F_STORE))
    {
      // if store addr known, operand not ready, and store addr matches load addr
      if (!OPERANDS_READY(&LSQ[jIndex]) && STORE_ADDR_READY(&LSQ[jIndex]) && rs->addr == LSQ[jIndex].addr)
      {          // oh no the store addr matches a load addr! we had a dependant store 
        numMemViolations++;
        // if a memory violation set the store set and the CDP

        //printf("numMemViolations %i\n", (int) numMemViolations);
        int iStoreSetIndex;
        ...
        for ( iStoreSetIndex = 0; iStoreSetIndex < numStoreSetIndices; iStoreSetIndex++)
        {
        	...
        	//CountingDependencePredictorSTATEUPDATE(CountingDependencePredictorIndex[iStoreSetIndex],CountingDependencePredictorIndex[iStoreSetIndex].match, int flush)
              // Found a Store Set Index with a Matching PC
              if ( StoreSetIndices[iStoreSetIndex].loadPC == rs->PC)
              {
                  // iStoreSetIndex -> is the StoreSetIndices index for this load.
                  foundStoreSetIndex = TRUE;
                  break;
        ...
        if (foundStoreSetIndex) 
        {
           StoreSetIndices[iStoreSetIndex].numStores++;
           int iStoreSet = StoreSetIndices[iStoreSetIndex].numStores;
           StoreSetIndices[iStoreSetIndex].storeSet[iStoreSet] = LSQ[jIndex].PC;
        }
        else // Initialize a Store Set Index for this load
        {
          StoreSetIndices[numStoreSetIndices].loadPC = rs->PC;
          StoreSetIndices[numStoreSetIndices].numStores++;
          int iStoreSet = StoreSetIndices[numStoreSetIndices].numStores;
          StoreSetIndices[numStoreSetIndices].storeSet[iStoreSet] = LSQ[jIndex].PC;
          numStoreSetIndices++;
        }
        ////////////////////////////// CDP //////////////////////////////////
                ///////////////////////////////// CDP search ////////////
        ...
        int iLoadPC =0;
      	...
      	{ // scan the CDP list for this load's CDP struct
      	  // PC was set in the memory violations function 
      		if(CountingDependencePredictorIndices[iLoadPC].loadPC == LSQ[jIndex].PC)
      		{ // if the LOAD has an entry in the CDP find the index and store it.
      			LoadINDEX = iLoadPC;
      		}
      	...
      	if (LoadINDEX > -1) // it exists
        {
           CountingDependencePredictorIndices[LoadINDEX].match++; // increase the number of matches
           CountingDependencePredictorSTATEUPDATE(&CountingDependencePredictorIndices[LoadINDEX],0); // update the state machine
           
        }
        /*else // Initialize a CDP Index for this load
        {
          CountingDependencePredictorIndices[numStoreSetIndices].loadPC = rs->PC;
          CountingDependencePredictorIndices[numStoreSetIndices].match++;
          CountingDependencePredictorSTATEUPDATE(&CountingDependencePredictorIndices[LoadINDEX],0); // update the state machine
          numCDPIndices++;
          if(numCDPIndices == MAX_NUM_CDPINDIES)
          {
        ...
        break;
      } // is store not ready, addr calculated, and matching
    } // if store
  } // for LSQ index
} // check_mem_violation
	  ...
	  if (rs->in_LSQ
	      ...
	      if (MD_OP_FUCLASS(rs->op) != NA)
		{
		  ...
		  if (fu)
			...
			{
        // ------------------------------------------------------------------------------------------------------------------------------
        //                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5: FUNCTION CALL
        // Description: Detect the presence of a queued load the address of which conflicts with an un-executed store.
        // ------------------------------------------------------------------------------------------------------------------------------
        check_mem_violation(rs);
...
// ------------------------------------------------------------------------------------------------------------------------------
//                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5: FUNCTION DEFINITION
// Name: dumpFinalProjectOutputs()
// Description: Write the following performance parameters to an output .bin file, named according to the memory prediction scheme selected.
//              numFalseDependencies, numMemViolations, IPC
//              All output .bin files generated by this program can be read in and visualized by the accompanying Matlab file, "visualize_MemDepPredPerf_Outputs.m"
// ------------------------------------------------------------------------------------------------------------------------------
static FILE *fidFinalProjOutputs;
static void
dumpFinalProjectOutputs()
{
  // Generate Output File Name Based on the Selected Memory Dependence Strategy
  char* outputFileName;
  if (ALGORITHM_TYPE == 1)
  {
    outputFileName = "./OutputData/outputsNoSpeculation.bin";
  }
  else if(ALGORITHM_TYPE == 2)
  {
    outputFileName = "./OutputData/outputsNaiveSpeculation.bin";
  }
  else if(ALGORITHM_TYPE == 3)
  {
    outputFileName = "./OutputData/outputsStoreSets.bin";
  }
  else if (ALGORITHM_TYPE == 4)
  {
    outputFileName = "./OutputData/outputsCDP.bin";
  }
  else
  { // fall back-- if nothing passed in as a parameter than default 
    outputFileName = "./OutputData/outputsDefault.bin";
  }
  // Compute IPC
  float simIPC = ((float)sim_num_insn)/((float)sim_cycle);
  // Initialize File to Write Final Project Stats To
  fidFinalProjOutputs = fopen(outputFileName, "wb");
  fwrite(&simIPC,               sizeof(float), 1, fidFinalProjOutputs); // IPC
  fwrite(&numFalseDependencies, sizeof(int),   1, fidFinalProjOutputs); // Number of False Dependencies
  fwrite(&numMemViolations,     sizeof(int),   1, fidFinalProjOutputs); // Number of Memory Violations
  fclose(fidFinalProjOutputs);
  return;
}
...
#include "machine.def"
  ...
  for (;;)
	...
	{
    // ------------------------------------------------------------------------------------------------------------------------------
    //                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5: ALGORITH SELECTION FEATURE
    // Description: Invoke the appropriate lsq_refresh*() based in the user's intended memory dependency scheme
    // ------------------------------------------------------------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------------------------------------------------------------
    //                                    [COMPARCH] COMPUTER ARCHITECTURE PROJECT 5: FUNCTION CALL
    // Description: Count the number of false dependencies. These are loads which are stalled (i.e. un-queued) due to downstream stores which do not actually conflict.
    // ------------------------------------------------------------------------------------------------------------------------------
    countNumFalseDependencies();
